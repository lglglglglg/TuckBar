...
> 项目状态：v0.9.6 Lloyd 式真实窗口转发 Alpha
> 创建日期：2026-09-07
...
锁屏/睡眠唤醒后菜单栏收起状态恢复见 [v0.8.8 唤醒后收起状态恢复](docs/17-v0.8.8唤醒后收起状态恢复.md)。

收纳篮打开时不展开原始菜单栏的修复见 [v0.8.9 收纳面板无闪现](docs/18-v0.8.9收纳面板无闪现.md)。

原始项目与收纳篮同时出现的状态栏重排竞态修复见 [v0.9.0 收纳面板竞态修复](docs/19-v0.9.0收纳面板竞态修复.md)。

锁屏唤醒后原始图标延迟收回的修复见 [v0.9.1 唤醒即时收起](docs/20-v0.9.1唤醒即时收起.md)。

收纳篮点击鼠标跳动、重复激活和点击后原始图标闪回的修复见 [v0.9.2 点击转发与并发收起修复](docs/21-v0.9.2点击转发与并发收起修复.md)。

单项目 Command 拖动改为单步状态切换、避免“鼠标画图式”动画见 [v0.9.3 iBar/Ice 交互收敛](docs/22-v0.9.3-iBar-Ice交互收敛.md)。

收纳篮点击改为通过辅助功能直接调用原始菜单栏项目，不再把图标拖回第一行，也不再注入鼠标移动/点击动画，见 [v0.9.4 AX 直达激活](docs/23-v0.9.4-AX直达激活.md)。

补充处理菜单栏项目在唤醒后 AX 子项重排、`CannotComplete` 重试，以及只提供 `ShowMenu` 动作的应用，见 [v0.9.5 AX 菜单动作兼容](docs/24-v0.9.5-AX菜单动作兼容.md)。该方案后来被撤回：在 macOS 26 上 AX 能找到项目但不一定能打开对应菜单。

收纳篮点击改为参考开源 [Lloyd](https://github.com/benwbooth/lloyd) 的真实窗口转发：只把被点击项目移动到可见区，使用带目标 PID/窗口 ID 的 CGEvent 完成点击，隐藏光标并等待窗口帧稳定后再收回。这样不会展开整组图标，也不会把鼠标拖拽动画暴露给用户，见 [v0.9.6 Lloyd 式真实窗口转发](docs/25-v0.9.6-Lloyd式真实窗口转发.md)。
